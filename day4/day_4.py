...
def my_solution2(test):
    ...
    for g in set(list_of_guards):
        my_list = my_np[list_of_guards == g]
        dict_of_sleep_times[g] = my_list.sum(axis=0)

    # find max and minute and guard
    max_g = ""
    max_m = 0
    max_times = 0
    for g in set(list_of_guards):
        m_t = max(dict_of_sleep_times[g])
        if m_t > max_times:
            max_g = g
            max_m = np.argmax(dict_of_sleep_times[g])
            max_times = max(dict_of_sleep_times[g])

    result = int(max_g[1:])*max_m
    return max_g, max_m, max_times, result
